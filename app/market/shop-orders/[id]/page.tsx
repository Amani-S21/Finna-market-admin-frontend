...
import {
  DelivererSelect,
  OrderStatusBadge
} from "../../orders/_components";
import {
  useFetchOrderById,
  useFetchShopOrderProducts,
} from "../../orders/_features/hooks";
import ShopOrderDetailsFeaturesTable from "../_components/ShopOrderDetailsFeaturesTable";
...
const BuildOrderDetailsPage = () => {
  ...
  if (isLoading || isLoadingShopOrders || status === "loading")
    return <LoadingOrderDetailsPage />;

  if (error || shopOrdersProductsError) notFound();
      ...
      <Grid columns="3" mt="4" gapX="4">
        <div className="col-span-2">
          ...
          <Text as="p" size="2" className="font-bold">
            Produits commandés pour la boutique
          </Text>
          <Text as="p" size="2" mt="1" mr="2">
            Tous les produits commandés par l'utilisateurs pour la boutique
          </Text>
          {shopOrdersProducts && (
            <ShopOrderDetailsFeaturesTable orderDetails={shopOrdersProducts} />
          )}
